...
- Send the payment screenshot to the bot in the chat, not through a file
  picker in the webview. Tapping **SEND MY SCREENSHOT IN TELEGRAM** makes the
  bot message you with the order code; reply to it with the picture and the
  snacks are yours. A Telegram webview's file picker behaves differently on
  every phone and on some of them opens nothing at all, which reads to a buyer
  as a shop that has taken their money and broken.
...
- Add items, edit prices, hide sold-out lines, create whole new categories,
  all from the phone with no redeploy.
- Promote other people to admin.
- Open/close the store and set the header announcement.

**Collation**
- Every order reaches Google Sheets when it is *placed*, not when somebody
  eventually verifies it: an `Orders` row, one `Order Items` row per line,
...
The bot also takes payment screenshots. Send a photo to the chat and it is
filed against your open order; if you have more than one, it asks which. A
screenshot sent as a file works too, and so does an iPhone HEIC, because what
the bytes actually are is checked rather than what the filename claims.
...
```
awaiting_payment ──screenshot to the bot──► pending_review ──admin approves──► paid ──► collected
       │                                          │
       │                                    admin rejects
       ├── buyer cancels ────► cancelled          ▼
       └── 45 min timeout ───► cancelled     rejected ──sends another──► pending_review
```

Stock is reserved from the moment the order is placed, and spent when the
screenshot arrives, because that is when the buyer walks off with the snacks.
An admin verifying the payment afterwards is bookkeeping, not a stock movement.
`cancelled` and an expiry release the reservation; a rejection cannot, because
by then the packets are gone. A janitor sweeps abandoned checkouts every five
minutes.

Every one of those arrows also refreshes the order's row in the spreadsheet
and, when it moved stock, the ledger tab and the low-stock alert. That is one
function, `queueStockFollowUp` in `services/collation.service.js`, so the
sheet cannot fall out of step with the shop one path at a time.
